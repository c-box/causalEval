...
class Decoder():
    def __init__(self, model, tokenizer,
                 init_method, iter_method,
                 MAX_ITER, BEAM_SIZE, NUM_MASK, BATCH_SIZE,
                 device=5,
                 verbose=True, length_norm_coeff=0.0):
        print(
            f"init_method={init_method} iter_method={iter_method} MAX_ITER={MAX_ITER} BEAM_SIZE={BEAM_SIZE} NUM_MASK={NUM_MASK} BATCH_SIZE={BATCH_SIZE}")
        ...
        self.length_norm_coeff = length_norm_coeff
    ...
    def decode_sentences(self, sentences):
        ...
        if torch.cuda.is_available():
            inp_tensor = inp_tensor.cuda(self.device)
            attention_mask = attention_mask.cuda(self.device)
            mask_ind = mask_ind.cuda(self.device)
        ...
        for b_out_tensor, b_logprob, b_mask_ind in zip(out_tensor, logprob, mask_ind):
            for i in range(self.NUM_MASK):
                ...
                for j in range(self.BEAM_SIZE):
                    ...
                    # length norm
                    lp = np.power(mask_len, self.length_norm_coeff)
                    prob = np.exp(log_prob / lp)
        ...
        for out_tensor, out_logprob in zip(out_tensors, out_logprobs):
            ...
            for b in range(beam_size):
                if init_method == 'independent':
                    ...
                    new_out_tensor = torch.zeros_like(out_tensor).scatter(1, seq_ind,
                                                                          new_out_tensors.gather(1, raw_ind))
